...
- CONTACT IS MANDATORY: every company MUST include a phone number OR an email — prefer a logistics, supply-chain, procurement, or operations contact. Search the company website, contact/about page, and public business directories. Only as a last resort, if the filters are so restrictive you cannot find enough matches with contact info, may you include the closest options without it.
...
export function buildMessagesRequest(params: ShippersParams, l: "en" | "fr") {
  const userPrompt = [
    `Find ${params.count} Canadian businesses (shippers) that match these filters and would be good freight-brokerage customers:`,
