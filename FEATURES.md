...
- **Added:** 2026-05-28
- **Status:** shipped
- **Description:** Second pass on the "usable menus" — per the reference login mockup, the cards/menus themselves now use a light off-white surface (`--color-menu-bg = #eef0f4`) with darker input pills nested inside (`--color-input-bg = #dce0e8`). Page bg, sidebar, header all stay dark slate. The `<Card>` primitive switched defaults; Label + Settings text colors retuned for contrast on the light surface. Lock screen card got the same treatment.
- **Files touched:** `src/index.css`, `src/components/ui/card.tsx`, `src/components/ui/label.tsx`, `src/components/LockScreen.tsx`, `src/pages/Settings.tsx`
...
- **Added:** 2026-05-28
- **Status:** shipped
- **Description:** First-class web build for GitHub Pages. `vite-plugin-pwa` wired with manifest (Mac/Chrome "Install app" → standalone window). `vite.config.ts` reads `GITHUB_PAGES=1` to switch the base path to `/paca-logistics-automations/`. New `.env.production` sets `VITE_DIRECT_ANTHROPIC=1` so the claude-client routes straight to api.anthropic.com instead of the (nonexistent) sidecar on Pages. Service worker generated by Workbox excludes `/api/*` and `/healthz` from cache fallback.
- **Files touched:** `vite.config.ts`, `.env.production`, `src/core/claude-client.ts`, `.github/workflows/pages.yml`, `package.json` (vite-plugin-pwa dev dep)
...
- **Added:** 2026-05-28
- **Status:** shipped
- **Description:** When `VITE_DIRECT_ANTHROPIC=1`, `claude-client.ts` skips the sidecar protocol and POSTs straight to `https://api.anthropic.com/v1/messages` with the in-memory `apiKey`, `anthropic-version: 2023-06-01`, and `anthropic-dangerous-direct-browser-access: true`. Same trade-off as the Tauri build (key visible in user's own DevTools network tab while in flight) — necessary for the static-site deploy.
- **Files touched:** `src/core/claude-client.ts`
